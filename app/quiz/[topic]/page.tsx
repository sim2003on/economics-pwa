...
export default function QuizPage({ params }: { params: { topic: string } }) {
  ...
  const [answers, setAnswers] = useState<string[]>(Array(topic?.quiz.length || 0).fill(""))
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <Card className="max-w-4xl mx-auto border-slate-200 dark:border-slate-800">
            <CardHeader className="border-b border-slate-200 dark:border-slate-800">
              <CardTitle className="text-3xl font-bold">Quiz Results: {topic.title}</CardTitle>
            </CardHeader>
            ...
            <CardContent className="pt-6">
              <div className="text-center mb-8">
                ...
                <p className="mt-2 text-slate-600 dark:text-slate-400">{percentage}% Correct</p>
              </div>
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 rounded-lg ${
                        isCorrect ? "bg-emerald-50 dark:bg-emerald-950/30" : "bg-rose-50 dark:bg-rose-950/30"
                      }`}
                    >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 dark:from-slate-950 dark:to-slate-900">
      <div className="container mx-auto px-4 py-8">
        <Button variant="ghost" className="mb-6" onClick={() => router.push(`/lesson/${params.topic}`)}>
        ...
        <Card className="max-w-4xl mx-auto border-slate-200 dark:border-slate-800">
          <CardHeader className="border-b border-slate-200 dark:border-slate-800">
            <div className="flex flex-col gap-4">
              <div className="flex justify-between items-center">
                <CardTitle className="text-2xl font-bold">{topic.title} Quiz</CardTitle>
          ...
          <CardFooter className="border-t border-slate-200 dark:border-slate-800 flex justify-end pt-6">
            ...
            >
              {currentQuestion < topic.quiz.length - 1 ? "Next Question" : "Finish Quiz"}
            </Button>
          </CardFooter>
        </Card>
      </div>
